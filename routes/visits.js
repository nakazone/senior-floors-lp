...
export async function createVisit(req, res) {
  try {
    ...
    const { lead_id, customer_id, project_id, scheduled_at, seller_id, technician_id, address, notes } = req.body;
    ...
    const [result] = await pool.execute(
      ...
      [lead_id || null, customer_id || null, project_id || null, scheduled_at,
       seller_id || null, technician_id || null, address || null, notes || null]
    );

    res.status(201).json({ success: true, data: { id: result.insertId }, message: 'Visit scheduled' });
  } catch (error) {
